...
class BaseQuantizer(nn.Module):
    ...
    def __init__(self, quant_config):
        ...
        # self.register_buffer('delta_out', None)
        ...
        self.soft_targets = quant_config.get('soft_targets', True)
    ...
    def rounding(self, x: torch.Tensor):
        ...
        # torch.clamp(torch.round(self.weight / self.weight_quantizer.delta) + self.weight_quantizer.zero_point, 0, self.weight_quantizer.n_levels - 1)
        # p(torch.isnan(torch.clamp(torch.round(self.weight / self.weight_quantizer.delta) + self.weight_quantizer.zero_point, 0, self.weight_quantizer.n_levels - 1)-self.weight_quantizer.zero_point).
# any())
        if self.sym:
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.init_done is not True:
            ...
            # self.delta_out = self.delta.clone()
    ...
    def init_quant_params(self, x: torch.Tensor, per_group: bool = False, momentum=False, n_bits=None):
        ...
        if self.scale_method == 'min_max':
            ...
            delta = scaled_max[0].clone()
            zero_point = scaled_max[0].clone()
            for i in range(0, x.shape[0]):
                x_q = self.quantize(x_ranged[:, i, :],scaled_max[:, i],scaled_min[:, i], n_bits, n_batch=n_step)
                lp_loss_ranged = lp_loss(x_ranged[:, i, :], x_q, p=2., reduction='none',n_batch=n_step)
                min_idx = torch.argmin(lp_loss_ranged)
                delta[i] = (scaled_max[min_idx, i]-scaled_min[min_idx, i])/(2**n_bits-1)
                zero_point[i] = (-scaled_min[min_idx, i]/delta[i]+eps).round()
                
            '''x_q = self.quantize(x_ranged,scaled_max,scaled_min, n_bits, n_batch=n_step)
    ...
    def quantize(self, x, x_max, x_min, n_bits, n_batch=None):
        ...
        eps=1.e-4
        delta = (x_max - x_min) / (2 ** n_bits - 1) if not self.always_zero else x_max / (2 ** n_bits - 1)
