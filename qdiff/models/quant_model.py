...
class QuantModel(nn.Module):
    ...
    def set_module_name_for_quantizer(self, module, prefix=""):
        ...
        for name_, module_ in module.named_children():
            ...
            if isinstance(module_, (BaseQuantizer, QuantLayer)):  # end with quantizer module
    ...
    def get_quant_params_dict(self, module=None, prefix="", dtype=torch.float32):
        ...
        for name, module_ in module.named_children():
            ...
            if isinstance(module_, quantizer_type):
                ...
                if isinstance(module_, QuantTemporalAttnLinear):
                    self.quant_params_dict[full_name + '.loraA.weight'] = module_.loraA.weight.data
                    self.quant_params_dict[full_name + '.loraB.weight'] = module_.loraB.weight.data
                    self.quant_params_dict[full_name + '.loraA_out.weight'] = module_.loraA_out.weight.data
                    self.quant_params_dict[full_name + '.loraB_out.weight'] = module_.loraB_out.weight.data
                    self.quant_params_dict[full_name + '.mask'] = module_.mask.data
                    self.get_quant_params_dict(module=module_, prefix=full_name+'.')
                elif isinstance(module_, QuantLayer):
                    self.quant_params_dict[full_name + '.loraA.weight'] = module_.loraA.weight.data
                    self.quant_params_dict[full_name + '.loraB.weight'] = module_.loraB.weight.data
                    self.quant_params_dict[full_name + '.loraA_out.weight'] = module_.loraA_out.weight.data
                    self.quant_params_dict[full_name + '.loraB_out.weight'] = module_.loraB_out.weight.data
    ...
    def set_quant_params_dict(self, quant_params_dict, module=None, load_buffer_only=True, dtype=torch.float32):
        ...
        for name, module_ in module.named_children():
            ...
            if isinstance(module_, quantizer_type):
               # unpack the dict
                if isinstance(module_, QuantTemporalAttnLinear):
                    module_.loraA.weight.data = quant_params_dict[module_.module_name+ '.loraA.weight']
                    module_.loraB.weight.data = quant_params_dict[module_.module_name + '.loraB.weight']
                    module_.loraA_out.weight.data = quant_params_dict[module_.module_name+ '.loraA_out.weight']
                    module_.loraB_out.weight.data = quant_params_dict[module_.module_name + '.loraB_out.weight']
                    module_.mask.data = quant_params_dict[module_.module_name + '.mask']
