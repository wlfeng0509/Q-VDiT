...
class STDiTBlock(nn.Module):
    ...
    def forward(self, x, y, t, mask=None, tpe=None):
        ...
        # mlp
        output = self.drop_path(gate_mlp * self.mlp(t2i_modulate(self.norm2(x), shift_mlp, scale_mlp)))
...
class STDiT(nn.Module):
    ...
    def forward(self, x, timestep, y, mask=None):
        ...
        if mask is not None:
            ...
            MASK_SELECT = True
            if hasattr(self.final_layer.linear, 'act_quantizer') and not isinstance(self.final_layer.linear.act_quantizer, DynamicActQuantizer): # static quant param
        ...
        for i, block in enumerate(self.blocks):
            if i == 0:
                ...
                tpe = None
            # print(i)
            x_ = auto_grad_checkpoint(block, x, y, t0, y_lens, tpe) # (i==len(self.blocks)-1)
